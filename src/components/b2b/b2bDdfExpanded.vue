    ...
    <TableCell colspan="8" class="max-w-0 p-0 overflow-x-auto">
      ...
      <div class="flex gap-4 bg-[#EFEFEF] p-4" style="min-width: max-content">
        ...
        <div class="flex flex-col gap-4 2xl:flex-row w-[320px] 2xl:w-full">
          ...
          <div class="flex flex-col overflow-hidden rounded-3xl border bg-white min-w-[280px] max-w-[280px]"
            ...
            <div class="px-6 py-5 flex flex-col gap-3">
              ...
              <div v-if="latestOrder && latestOrder.order_status === 'order_requested'"
            ...
            <div class="flex-1 px-6 pb-5">
              <div v-for="(entry, i) in vehicle.timeline || []" :key="i" class="relative flex items-start pb-6">
                <!-- Vertical line -->
                <div v-if="i < (vehicle.timeline || []).length - 1" class="absolute left-2 top-5 w-0.5 h-full" :style="i >= (vehicle.timeline || []).length - 1
                  ? 'background:#01B990'
                  : 'background:#B7C2C2'
                  " />

                <!-- Dot -->
                <div class="relative z-10 w-4 h-4 shrink-0 rounded-full mt-1" :style="i >= (vehicle.timeline || []).length - 1
                  ? 'background:#01B990'
                  : 'background:#B7C2C2'
                  " />
            ...
            <div class="relative flex flex-col rounded-[16px] border bg-white 2xl:h-full" style="border-color: #ececec">
              ...
              <div class="flex flex-col gap-4 px-6 pt-4 pb-4">
                <div v-for="group in groupedDocuments" :key="group.key" class="flex flex-col gap-3">
                  ...
                  <div v-for="(doc, i) in group.items" :key="doc.id || i"
        ...
        <div class="flex flex-col gap-4 shrink-0" style="width: 400px">
          <div class="relative">
            <div class="flex flex-col rounded-[16px] border bg-white"
  ...
  <div v-if="pendingOfferId"
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" @click.stop>
      ...
      <div class="mt-6 flex justify-end gap-3">
        <button @click="cancelSelect" :disabled="selectingOfferId !== null"
        ...
        <button @click="confirmSelect" :disabled="selectingOfferId !== null"
