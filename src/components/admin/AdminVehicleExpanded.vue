  ...
  <TableRow class="border-0 hover:bg-transparent">
    <TableCell colspan="8" class="max-w-0 p-0 overflow-x-auto">
      <div class="flex gap-2.5 border-t border-[#D9E2E2] bg-white p-4 overflow-x-auto">
        ...
        <div class="flex w-43 shrink-0 flex-col gap-4">
          <div class="rounded-[5px] border border-[#ECECEC] p-3">
            <p class="text-base font-bold text-[#2E3E3F] mb-3">
              Firma
              <span class="text-[14px] font-normal text-[#2E3E3F]"> / Firmenlogo </span>
            </p>
            <div class="aspect-square w-full bg-[#f3f4f6] rounded flex items-center justify-center">
        ...
        <div
          class="flex w-71 shrink-0 flex-col rounded-[10px] border border-[#ECECEC] bg-white p-6"
        >
          <p class="text-[20px] font-bold text-[#2E3E3F] mb-8">Status bei: ATU Lüneburg</p>
          <div class="flex flex-col">
            <div v-for="(entry, i) in timeline" :key="i" class="flex items-stretch min-h-15">
              <!-- Date column -->
              ...
              <div class="relative flex w-6 shrink-0 flex-col items-center">
                ...
                <div
                  ...
                  :class="
                    timeline[i + 1].status === 'completed' ? 'bg-green-gray' : 'bg-green-gray'
                  "
                ></div>
                <!-- Dot -->
                <div
                  class="size-2 rounded-full z-10 mt-3"
                  :class="entry.status === 'completed' ? 'bg-green-gray' : 'bg-green-gray'"
                ></div>
              </div>
              ...
              <div class="flex-1 pl-4 pb-6">
                <div v-if="entry.label === 'DEKRA'" class="flex flex-col">
                  <div class="flex items-center gap-1.5">
                    <Icon icon="ph:play-fill" class="size-4 text-custom-green" />
                    <span class="text-sm font-bold text-custom-green">DEKRA</span>
                  </div>
                  <p class="text-sm text-[#2E3E3F] leading-tight whitespace-pre-line mt-1">
                    {{ entry.sublabel }}
                  </p>
                </div>
                <div v-else class="pt-1">
                  <p class="text-sm text-[#2E3E3F] leading-tight whitespace-pre-line font-normal">
                    {{ entry.label }}
                  </p>
                </div>
              </div>
        ...
        <div class="flex w-81 shrink-0 flex-col rounded-[5px] border border-[#ECECEC] bg-white p-4">
          ...
          <div class="flex flex-col gap-4">
            <div v-for="offer in offers" :key="offer.id" class="flex flex-col gap-1">
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div
                    ...
                    :class="offer.accepted ? 'border-[#EF8450]' : 'border-green-gray'"
                  >
                    <div v-if="offer.accepted" class="size-2 rounded-full bg-[#EF8450]"></div>
                  </div>
                </div>
              </div>
          ...
          <button class="mt-4 w-full bg-green-gray text-white py-2 rounded font-bold text-[14px]">
            Im Auftrag des Klienten Annehmen
          </button>

          <div class="mt-4 border border-green-gray rounded p-2">
            <p class="text-[13px] font-bold text-[#2E3E3F] mb-1">Angenommenes Angebot</p>
            <div class="bg-gray-50 p-1 text-[12px] text-gray-400">Noch kein Angebot vorhanden</div>
          </div>
          ...
          <div class="mt-6 flex justify-center gap-6">
            ...
            <div class="flex flex-col items-center gap-2">
              <div class="flex gap-1">
                ...
                >
                  <span class="text-[30px] font-bold text-white leading-none">{{ digit }}</span>
                  <div
        ...
        <!-- Column 4: Dokumente -->
        <div
          class="flex w-54.5 shrink-0 flex-col rounded-[5px] border border-[#ECECEC] bg-white p-4 relative"
        >
          <button class="absolute right-0.5 top-0.5 text-custom-green">
            <Icon icon="mdi:pencil-outline" class="size-5" />
          </button>
          <p class="text-[16px] font-bold text-[#2E3E3F] mb-6">Fahrzeug Dokumente</p>
          <div class="flex flex-col gap-4 mb-8">
            ...
            >
              ...
              <Icon icon="mdi:file-download-outline" class="size-5 text-custom-green" />
            </div>
          </div>
          ...
          <p class="text-[14px] font-bold text-[#2E3E3F] mb-4">Rückgabe Dokumente</p>
        ...
        <div
          class="flex w-37.5 shrink-0 flex-col rounded-[5px] border border-[#ECECEC] bg-white p-4 relative"
        >
          <button class="absolute right-0.5 top-0.5 text-custom-green">
            <Icon icon="mdi:pencil-outline" class="size-5" />
          </button>
          <p class="text-[16px] font-bold text-[#2E3E3F] mb-6">Fahrzeug Daten</p>
          <div class="flex flex-col gap-4">
            <div>
        ...
        <!-- Column 6: Zugewiesen -->
        <div
          class="flex w-50 shrink-0 flex-col rounded-[5px] border border-[#ECECEC] bg-white p-4 relative"
        >
          <button class="absolute right-0.5 top-0.5 text-custom-green">
            <Icon icon="mdi:pencil-outline" class="size-5" />
          </button>
          ...
          <div class="flex flex-col gap-2">
            <p class="text-[13px]"><span class="font-bold">Vorname:</span> Marcus</p>
            <p class="text-[13px]"><span class="font-bold">Nachname:</span> Dietrich</p>
            <p class="text-[13px]"><span class="font-bold">Tel.:</span> 17655874354</p>
            <p class="text-[13px] font-bold">Nutzungsadresse:</p>
            <p class="text-[12px] text-[#2E3E3F]">Radestraße 12, 35037 Marburg</p>
            <div class="mt-2">
