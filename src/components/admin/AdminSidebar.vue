...
const nav = [
  {
    id: 'dashboard', label: 'Dashboard', name: 'admin',
  ...
  {
    id: 'kunden', label: 'Kunden', name: 'admin-kunden',
  ...
  {
    id: 'fahrzeuge', label: 'Fahrzeuge', name: 'admin-fahrzeuge',
  ...
  {
    id: 'auftraege', label: 'Alle Aufträge', name: 'admin-auftraege',
...
const userRoleLabel = computed(() => {
  ...
  return ({ ADMIN: 'Administrator', Admin: 'Administrator', B2B: 'B2B Nutzer', B2C: 'B2C Nutzer' } as any)[r] ?? r
})

function isActive(name: string) { return route.name === name }
function navigateTo(name: string) { void router.push({ name }) }
...
<template>
  ...
  -->
  <aside
    class="sticky top-0 h-screen flex flex-col py-5 rounded-[22px] shrink-0 z-50
           transition-[width] duration-300 ease-out"
    :class="collapsed ? 'w-[78px] px-3' : 'w-[224px] px-4'"
    style="background:linear-gradient(180deg,#10393b 0%,#0d3133 100%);
           box-shadow:0 8px 30px rgba(16,57,59,0.18);
           overflow:visible;"
  >

    <!-- ── BRAND ── -->
    <div class="flex items-center gap-2.5 px-1 h-10 mb-7 shrink-0 overflow-hidden">
      <div
        class="w-10 h-10 rounded-[13px] flex items-center justify-center shrink-0"
        style="background:linear-gradient(150deg,#01B990,#10393b);box-shadow:0 4px 12px rgba(1,185,144,0.3)"
      >
      ...
      <transition name="lb-fade">
        <div v-if="!collapsed" class="flex flex-col leading-none whitespace-nowrap overflow-hidden">
          <span class="text-[16px] font-extrabold text-white tracking-[-0.4px]">
            Leasy<span class="text-[#01B990]">Back</span>
          </span>
          <span class="text-[9.5px] font-bold text-white/35 uppercase tracking-[0.1em] mt-1">Admin</span>
        </div>
      </transition>
    </div>
    ...
    <button
      ...
      class="absolute -right-3 top-7 z-[60] w-6 h-6 rounded-full bg-white border border-[#d8e4e3]
             flex items-center justify-center text-[#6f8585]
             hover:text-[#10393b] hover:scale-110 hover:border-[#01B990] transition-all duration-200"
    ...
    <nav class="flex-1 flex flex-col gap-0.5 overflow-y-auto overflow-x-hidden min-h-0 pr-0.5">
      ...
      <button
        ...
        class="group relative flex items-center gap-3 h-[44px] rounded-[13px]
               transition-all duration-150 shrink-0 w-full text-left"
        :class="[
      ...
      >
        ...
        <span
          v-if="collapsed"
          class="absolute left-[58px] whitespace-nowrap pointer-events-none
                 bg-[#10393b] text-white text-[12px] font-semibold
                 px-2.5 py-1.5 rounded-lg border border-white/10
    ...
    <div class="shrink-0 pt-3 mt-3 overflow-hidden" style="border-top:1px solid rgba(255,255,255,0.08)">

      <!-- Logout -->
      <button
        @click="logout"
        class="group relative flex items-center gap-3 w-full rounded-[13px] mb-1.5
               text-white/35 hover:bg-white/[0.06] hover:text-white/75 transition-all"
      ...
      <!-- User pill -->
      <div
        class="flex items-center gap-2.5 rounded-[13px] hover:bg-white/[0.06]
               transition-all cursor-pointer"
        :class="collapsed ? 'justify-center p-1.5' : 'p-2'"
      >
        <div
          class="w-9 h-9 rounded-[11px] text-white text-[12px] font-extrabold
