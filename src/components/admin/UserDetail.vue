...
async function fetchVehicles() {
  ...
  try {
    const res = await adminVehiclesApi.listByUser(
      props.user.user_id, vehiclePage.value, vehicleLimit.value
    )
...
async function fetchOrders() {
  ...
  try {
    const res = await adminOrdersApi.listByUser(
      props.user.user_id, orderPage.value, orderLimit.value
    )
  ...
  <Teleport to="body">
    <Transition name="lb-modal">
      <div
        ...
        style="background: rgba(16,57,59,0.5); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);"
      ...
      >
        ...
        <div
          ...
          style="max-height: 90vh; box-shadow: 0 32px 80px rgba(16,57,59,0.25), 0 0 0 1px rgba(16,57,59,0.06);"
          @click.stop
        >
          ...
          >
            <!-- Avatar -->
            <div
              class="w-[52px] h-[52px] rounded-[15px] flex items-center justify-center
                     text-[18px] font-extrabold text-[#10393b] shrink-0"
              style="background: linear-gradient(150deg, #01B990, #5fe6c2);
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[10.5px] font-bold text-[#01B990] uppercase tracking-[0.14em] mb-1">Kundenprofil</p>
              <h2 class="text-[21px] font-extrabold text-white tracking-[-0.4px] leading-tight truncate">
            ...
            <button
              @click="close"
              class="w-9 h-9 flex items-center justify-center rounded-[11px] shrink-0 ml-1
          ...
          <!-- ━━━ INFO STRIP ━━━ -->
          <div class="grid grid-cols-3 shrink-0" style="border-bottom: 1px solid #eef3f2">
            <!-- Address -->
            <div class="px-6 py-4" style="border-right: 1px solid #eef3f2">
              <p class="text-[10px] font-bold text-[#9bb0af] uppercase tracking-[0.12em] mb-1.5">Adresse</p>
            ...
            <!-- Country -->
            <div class="px-6 py-4" style="border-right: 1px solid #eef3f2">
              <p class="text-[10px] font-bold text-[#9bb0af] uppercase tracking-[0.12em] mb-1.5">Land</p>
              <p class="text-[13px] font-semibold text-[#10393b]">{{ user.country || '—' }}</p>
              <p class="text-[12px] text-[#6f8585]">{{ user.additional_address || '—' }}</p>
            </div>
            <!-- Meta -->
            <div class="px-6 py-4">
              <p class="text-[10px] font-bold text-[#9bb0af] uppercase tracking-[0.12em] mb-1.5">Beigetreten</p>
              <p class="text-[13px] font-semibold text-[#10393b]">{{ formatGermanDate(user.created_at) }}</p>
          ...
          <div class="flex items-center gap-2 px-7 pt-5 pb-3 shrink-0">
            <button
              ...
              class="flex items-center gap-2 px-4 py-2 rounded-[11px] text-[13px] font-bold
                     transition-all duration-150 font-[Manrope,sans-serif]"
          ...
          <div class="flex-1 overflow-y-auto px-7 pb-7 min-h-0">
            ...
            <div v-show="activeTab === 'vehicles'">
              ...
              <div v-if="vehicleLoading" class="flex flex-col gap-3 pt-1">
                <div v-for="i in 4" :key="i" class="h-[66px] rounded-[16px] animate-pulse" style="background:#f4f7f6"></div>
              </div>

              <!-- error -->
              <div v-else-if="vehicleError" class="py-8 text-center text-[13px]" style="color:#ef4444">
                {{ vehicleError }}
              </div>

              <!-- empty -->
              <div v-else-if="vehicles.length === 0" class="py-14 text-center">
                <div class="w-14 h-14 rounded-[18px] mx-auto mb-4 flex items-center justify-center" style="background:rgba(16,57,59,0.07); color:#10393b">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M5 17H3a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v9a2 2 0 01-2 2h-1"/><circle cx="9" cy="17" r="2"/><circle cx="17" cy="17" r="2"/></svg>
                </div>
                <p class="text-[13px] font-semibold" style="color:#9bb0af">Keine Fahrzeuge gefunden</p>
              </div>

              <!-- list -->
              <div v-else class="flex flex-col gap-2.5">
                <div
                  v-for="v in vehicles" :key="v.vehicle_id"
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-0.5 flex-wrap">
                      <span class="text-[14px] font-bold" style="color:#10393b">{{ v.make }} {{ v.model }}</span>
                      <span class="text-[11px] font-semibold" style="color:#9bb0af">{{ v.license_plate }}</span>
                    </div>
                    <div class="flex items-center gap-3 flex-wrap" style="font-size:12px; color:#6f8585">
                      <span>VIN: <span class="font-mono">{{ v.vin }}</span></span>
              ...
              <!-- vehicle pagination -->
              <div v-if="vehicleTotalPages > 1 && !vehicleLoading" class="flex items-center justify-between mt-5">
                <span class="text-[12px] font-medium" style="color:#9bb0af">
                  Seite {{ vehiclePage }} von {{ vehicleTotalPages }}
                </span>
                <div class="flex gap-1">
                  <button @click="vehiclePage--" :disabled="vehiclePage <= 1" class="pg-btn">←</button>
                  <button
                    v-for="p in pageRange(vehiclePage, vehicleTotalPages)" :key="String(p)"
                    @click="typeof p === 'number' && (vehiclePage = p)"
                    class="pg-btn"
                    :class="{ 'pg-active': p === vehiclePage, 'pg-ellipsis': p === '…' }"
                  >{{ p }}</button>
                  <button @click="vehiclePage++" :disabled="vehiclePage >= vehicleTotalPages" class="pg-btn">→</button>
                </div>
              </div>
            </div>
            ...
            <div v-show="activeTab === 'orders'">
              ...
              <!-- error -->
              <div v-else-if="orderError" class="py-8 text-center text-[13px]" style="color:#ef4444">
                {{ orderError }}
              </div>

              <!-- empty -->
              <div v-else-if="orders.length === 0" class="py-14 text-center">
                <div class="w-14 h-14 rounded-[18px] mx-auto mb-4 flex items-center justify-center" style="background:rgba(99,102,241,0.1); color:#6366f1">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6M16 13H8M16 17H8"/></svg>
                </div>
                <p class="text-[13px] font-semibold" style="color:#9bb0af">Keine Aufträge gefunden</p>
              </div>
              ...
              <div v-else class="flex flex-col gap-2.5">
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-0.5 flex-wrap">
                      <span class="text-[14px] font-bold font-mono" style="color:#10393b">{{ o.auftragsnummer }}</span>
                      <span class="text-[12px]" style="color:#9bb0af">{{ o.make }} {{ o.model }}</span>
                    </div>
                    <div class="flex items-center gap-3 flex-wrap" style="font-size:12px; color:#6f8585">
              ...
              <!-- order pagination -->
              <div v-if="orderTotalPages > 1 && !orderLoading" class="flex items-center justify-between mt-5">
                <span class="text-[12px] font-medium" style="color:#9bb0af">
                ...
                <div class="flex gap-1">
                  ...
                  <button
                    v-for="p in pageRange(orderPage, orderTotalPages)" :key="String(p)"
                    @click="typeof p === 'number' && (orderPage = p)"
                    class="pg-btn"
                    :class="{ 'pg-active': p === orderPage, 'pg-ellipsis': p === '…' }"
                  >{{ p }}</button>
                  <button @click="orderPage++" :disabled="orderPage >= orderTotalPages" class="pg-btn">→</button>
                </div>
              </div>
            </div>
...
<style scoped>
/* Modal transition */
.lb-modal-enter-active { transition: all 0.24s cubic-bezier(0.16,1,0.3,1); }
.lb-modal-leave-active { transition: all 0.18s ease; }
.lb-modal-enter-from,
.lb-modal-leave-to    { opacity: 0; }
.lb-modal-enter-from .relative,
.lb-modal-leave-to .relative { transform: scale(0.95) translateY(12px); }
...
.pg-btn:hover:not(:disabled):not(.pg-ellipsis) {
  border-color: #10393b; color: #10393b;
}
.pg-btn:disabled { opacity: 0.35; cursor: not-allowed; }
.pg-btn.pg-active { background: #10393b; border-color: #10393b; color: white; }
.pg-btn.pg-ellipsis { border-color: transparent; cursor: default; color: #9bb0af; }
</style>