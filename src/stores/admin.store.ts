...
export const useAdminStore = defineStore('admin', () => {
  ...
  async function fetchUsers(type?: 'b2c' | 'b2b', page?: number) {
    ...
    try {
      users.value = usersType.value === 'b2b'
  ...
  async function fetchVehicles(opts?: {
    ...
    try {
      vehicles.value = vehiclesUserType.value === 'all'
  ...
  async function fetchOrders(opts?: {
    ...
    try {
      orders.value = ordersUserType.value === 'all'
  ...
  return {
    ...
    totalCustomers, totalB2C, totalB2B, totalVehicles,
    totalOrders, activeOrders, completedOrders, pendingInspections,

    // users
    users, usersLoading, usersError, usersPage, usersLimit, usersType,
    fetchUsers,

    // vehicles
    vehicles, vehiclesLoading, vehiclesError,
    vehiclesPage, vehiclesLimit, vehiclesUserType, vehiclesStatus,
    fetchVehicles, fetchVehiclesByUser,

    // orders
    orders, ordersLoading, ordersError,
    ordersPage, ordersLimit, ordersUserType,
    fetchOrders, fetchOrdersByUser,
