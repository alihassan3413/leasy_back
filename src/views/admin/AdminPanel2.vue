...
const sectionCards = computed(() => [
  {
    ...
    value: summary.value ? summary.value.total_b2c_customers + summary.value.total_b2b_customers : 0,
...
async function loadUsers() {
  ...
  try {
    const response = userType.value === 'B2C'
...
async function loadVehicles() {
  ...
  try {
    const response = await adminVehiclesApi.listByUserType(vehicleUserType.value, page.value, limit.value, vehicleStatus.value || undefined)
...
watch([activeSection, page, userType, vehicleUserType, orderUserType, vehicleStatus, orderStatus], () => {
...
<template>
  <div class="space-y-6">
    <header class="flex flex-col gap-3 rounded-[24px] border border-[#e8f1ef] bg-white p-6 shadow-[0_8px_30px_rgba(16,57,59,0.08)]">
      <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <div class="text-[12px] font-bold uppercase tracking-[0.2em] text-[#01B990]">Admin Übersicht</div>
          <h1 class="mt-2 text-[32px] font-extrabold text-[#10393b]">Flotten- und Kundenübersicht</h1>
          <p class="mt-2 text-base text-[#5a6e6c]">Live-Daten für Kunden, Fahrzeuge und Aufträge. Klicken Sie eine KPI-Karte, um die Liste unten anzuzeigen.</p>
        </div>
      </div>
    </header>
    ...
    <section class="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
      <div class="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        <template v-for="card in sectionCards" :key="card.id">
          <button
            type="button"
            @click="handleCardClick(card.id === 'completed' || card.id === 'inspections' ? 'orders' : card.id as 'users' | 'vehicles' | 'orders')"
            class="group rounded-[24px] border p-6 text-left transition-all duration-200"
            :class="isCardActive(card.id)
          ...
          >
            <div class="flex items-center justify-between gap-3">
              <p class="text-[13px] font-semibold uppercase tracking-[0.18em] text-[#5a6e6c]">{{ card.label }}</p>
              <span class="inline-flex h-10 min-w-[58px] items-center justify-center rounded-full bg-[#e8faf3] px-3 text-sm font-bold text-[#05603a]">Live</span>
            </div>
            <div class="mt-6 flex items-end justify-between gap-4">
              ...
              <div class="h-14 w-14 rounded-[18px] bg-[#01B990]/10 text-[#01B990] flex items-center justify-center text-xl font-extrabold">{{ card.label[0] }}</div>
            </div>
          </button>
        </template>
      </div>

      <div class="rounded-[24px] border border-[#eef3f2] bg-white p-6 shadow-[0_10px_28px_rgba(16,57,59,0.08)]">
        <div class="flex items-center justify-between gap-3">
          <div>
            <p class="text-[13px] font-semibold uppercase tracking-[0.18em] text-[#01B990]">Letzte Aufträge</p>
            <h2 class="mt-2 text-[20px] font-extrabold text-[#10393b]">Aktuelle Rückgaben</h2>
          </div>
          <span class="rounded-full bg-[#f3faf8] px-3 py-1 text-sm font-semibold text-[#05603a]">7 Einträge</span>
        </div>

        <div class="mt-5 space-y-3">
          <template v-if="recentOrders.length">
            <div v-for="order in recentOrders" :key="order.id" class="rounded-[20px] border border-[#e8f1ef] bg-[#fdfdfd] p-4 hover:bg-[#f7fbfa] transition">
              <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                  <p class="text-sm font-bold text-[#10393b]">{{ order.auftragsnummer }}</p>
                  <p class="text-sm text-[#5a6e6c]">{{ order.make }} {{ order.model }} • {{ order.license_plate }}</p>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  />
                  <span class="text-sm text-[#6f8585]">{{ formatGermanDate(order.created_at) }}</span>
                </div>
              </div>
            </div>
          </template>
    ...
    <section class="rounded-[24px] border border-[#eef3f2] bg-white p-6 shadow-[0_10px_28px_rgba(16,57,59,0.08)]">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p class="text-[13px] font-semibold uppercase tracking-[0.18em] text-[#01B990]">{{ activeLabel }}</p>
      ...
      <div class="mt-5 flex flex-wrap items-center gap-3">
        <span class="rounded-full bg-[#f3faf8] px-4 py-2 text-sm font-semibold text-[#05603a]">{{ total }} Gesamt</span>
        <span class="rounded-full bg-[#eff6ff] px-4 py-2 text-sm font-semibold text-[#1e3a8a]">{{ totalActive }} Aktiv</span>
        <span class="rounded-full bg-[#ffedde] px-4 py-2 text-sm font-semibold text-[#c2410c]">{{ totalInactive }} Inaktiv</span>
          ...
          <tbody class="divide-y divide-[#eef3f2]">
            <template v-if="loadingList">
              <tr v-for="index in 5" :key="index" class="animate-pulse">
                <td colspan="5" class="py-6 px-4"><div class="h-4 w-full rounded-full bg-[#e2e8f0]"></div></td>
              </tr>
            </template>

            <template v-else-if="activeSection === 'users'">
              <tr v-if="!users.length">
                <td colspan="5" class="py-8 px-4 text-center text-sm text-[#6f8585]">Keine Benutzer gefunden.</td>
              </tr>
              <tr v-for="user in users" :key="user.user_id" class="hover:bg-[#f7faf8] transition">
                <td class="px-4 py-4">
                  <div class="font-semibold text-[#10393b]">{{ user.salutation }} {{ user.first_name }} {{ user.last_name }}</div>
                ...
                <td class="px-4 py-4 text-sm text-[#6f8585]">{{ formatGermanDate(user.created_at) }}</td>
                <td class="px-4 py-4 text-sm text-[#6f8585]">{{ user.city }}, {{ user.country }}</td>
              </tr>
            </template>

            <template v-else-if="activeSection === 'vehicles'">
              <tr v-if="!vehicles.length">
                <td colspan="5" class="py-8 px-4 text-center text-sm text-[#6f8585]">Keine Fahrzeuge gefunden.</td>
              </tr>
              <tr v-for="vehicle in vehicles" :key="vehicle.vehicle_id" class="hover:bg-[#f7faf8] transition">
                <td class="px-4 py-4">
                  <div class="font-semibold text-[#10393b]">{{ vehicle.license_plate }}</div>
                  <div class="text-sm text-[#6f8585]">VIN {{ vehicle.vin }}</div>
                </td>
                <td class="px-4 py-4 text-sm text-[#10393b]">{{ vehicle.make }} {{ vehicle.model }}</td>
                <td class="px-4 py-4">
                  <StatusBadge
                    :label="getVehicleStatusLabel(vehicle.current_order_status || 'completed').label"
                    :colorClass="getVehicleStatusLabel(vehicle.current_order_status || 'completed').colorClass"
                    :backgroundClass="getVehicleStatusLabel(vehicle.current_order_status || 'completed').backgroundClass"
                  />
                </td>
                <td class="px-4 py-4 text-sm text-[#6f8585]">{{ formatGermanDate(vehicle.leasing_end_date) }}</td>
                <td class="px-4 py-4 text-sm text-[#6f8585]">{{ vehicle.user_type }} / {{ vehicle.company_name ?? userType }}</td>
              </tr>
            </template>

            <template v-else>
              <tr v-if="!orders.length">
                <td colspan="5" class="py-8 px-4 text-center text-sm text-[#6f8585]">Keine Aufträge gefunden.</td>
              </tr>
              <tr v-for="order in orders" :key="order.id" class="hover:bg-[#f7faf8] transition">
                ...
                <td class="px-4 py-4 text-sm text-[#6f8585]">{{ formatGermanDate(order.created_at) }}</td>
                <td class="px-4 py-4 text-sm text-[#6f8585]">{{ order.company_name || order.user_email }}</td>
              </tr>
            </template>
          </tbody>
        </table>
      ...
      <div v-if="listError" class="mt-4 rounded-[18px] border border-red-200 bg-red-50 p-4 text-sm text-red-700">{{ listError }}</div>
    </section>
  </div>
</template>
