...
async function loadOrders() {
  ...
  try {
    const res = userType.value === 'all'
      ? await adminOrdersApi.listAll(page.value, limit.value)
      : await adminOrdersApi.listByUserType(userType.value, page.value, limit.value)
...
watch([userType, page], () => void loadOrders())
onMounted(() => void loadOrders())

// ── Client-side status filter (API doesn't accept status on orders) ─
...
<template>
  <div class="flex flex-col gap-5 h-full">
    ...
    >
      <div class="flex-1">
        <h1 class="text-[18px] font-extrabold text-[#10393b] tracking-[-0.4px]">Auftragsverwaltung</h1>
      </div>
    ...
    >
      ...
      <div class="flex items-start justify-between mb-4 shrink-0 gap-4 flex-wrap">
        <div>
          <h2 class="text-[20px] font-extrabold text-[#10393b] tracking-[-0.4px]">
            {{
              userType === 'all' ? 'Alle Aufträge'
              : userType === 'Firmenkunde' ? 'Firmenkunden Aufträge'
      ...
      <div class="flex-1 overflow-auto rounded-[18px] border border-[#eef3f2] min-h-0">
        <table class="min-w-full border-collapse">
          <thead class="sticky top-0 z-10">
            <tr class="bg-[#f8faf9]">
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Auftrag</th>
              ...
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Partner</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Status</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">API</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Erstellt</th>
            </tr>
          </thead>
          <tbody>
            ...
            >
              ...
              <!-- Fahrzeug -->
              <td class="px-5 py-3.5">
                <div class="text-[13px] font-semibold text-[#10393b]">{{ o.make }} {{ o.model }}</div>
                <div class="text-[11px] text-[#9bb0af] mt-0.5 font-mono">{{ o.license_plate }}</div>
              </td>

              <!-- Kunde -->
              <td class="px-5 py-3.5">
                <div class="text-[13px] text-[#5a6e6c]">{{ o.company_name ?? o.user_email ?? '—' }}</div>
                <div class="text-[11px] text-[#9bb0af] mt-0.5">{{ o.user_type }}</div>
              </td>

              <!-- Partner -->
              <td class="px-5 py-3.5">
                <span
                  class="inline-flex items-center text-[11.5px] font-bold px-2.5 py-1 rounded-[8px]"
                  style="background:rgba(16,57,59,0.06); color:#10393b"
                >{{ o.leasyback_partner }}</span>
              </td>
              ...
              <td class="px-5 py-3.5">
                <span
                  class="inline-flex items-center text-[11.5px] font-bold px-2 py-0.5 rounded-[6px] tabular-nums"
      ...
      <div class="flex items-center justify-between mt-4 shrink-0">
        <span class="text-[12px] text-[#9bb0af] font-medium">
          ...
          <span v-if="statusFilter" class="ml-2 text-[#10393b] font-bold">
