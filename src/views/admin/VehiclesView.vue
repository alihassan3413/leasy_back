      ...
      <div class="flex items-start justify-between mb-4 shrink-0 gap-4 flex-wrap">
        <div>
          <h2 class="text-[20px] font-extrabold text-[#10393b] tracking-[-0.4px]">
            {{ userType === 'all' ? 'Alle Fahrzeuge' : userType === 'Firmenkunde' ? 'Firmenkunden Fahrzeuge' : 'Privatkunden Fahrzeuge' }}
          </h2>
      ...
      <div class="flex-1 overflow-auto rounded-[18px] border border-[#eef3f2] min-h-0">
        <table class="min-w-full border-collapse">
          <thead class="sticky top-0 z-10">
            <tr class="bg-[#f8faf9]">
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Fahrzeug</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Kennzeichen / VIN</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Kunde</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Auftragsstatus</th>
              <th class="text-left text-[11px] font-bold text-[#9bb0af] uppercase tracking-[0.1em] px-5 py-3.5 border-b border-[#eef3f2]">Leasingende</th>
            ...
            <template v-else v-for="v in vehicles" :key="v.vehicle_id">
              ...
              >
                ...
                <td class="px-3 py-3.5">
                  <span
                    class="w-8 h-8 flex items-center justify-center rounded-[9px]
              ...
              <Transition name="accordion">
                <tr v-if="expandedId === v.vehicle_id">
                  <td colspan="6" class="px-5 pb-4 pt-0 bg-[#f8faf9]">
                    <div class="rounded-[16px] border border-[#eef3f2] bg-white overflow-hidden">

                      <!-- Current order highlight -->
                      <div v-if="v.current_auftragsnummer" class="flex items-center gap-4 px-5 py-3.5 border-b border-[#eef3f2]">
                        <div class="w-1.5 h-1.5 rounded-full bg-[#01B990] shrink-0"></div>
                        <div class="flex-1">
                          <span class="text-[12px] font-bold text-[#9bb0af] uppercase tracking-[0.08em]">Aktueller Auftrag</span>
                          <span class="ml-3 text-[13px] font-extrabold text-[#10393b] font-mono">
                        ...
                        <span class="text-[12px] text-[#9bb0af] tabular-nums">
                          {{ v.current_order_created_at ? formatGermanDate(v.current_order_created_at) : '' }}
                        </span>
                      </div>

                      <!-- Order history list -->
                      <div v-if="v.order_history?.length">
                        <div class="px-5 py-2.5 bg-[#f8faf9] border-b border-[#eef3f2]">
                          <span class="text-[10.5px] font-bold text-[#9bb0af] uppercase tracking-[0.1em]">
                            Auftragshistorie ({{ v.order_history.length }})
                          </span>
                        </div>
                        <div
                          v-for="(h, idx) in v.order_history" :key="h.id"
                          class="flex items-center gap-4 px-5 py-3"
                          :class="idx < v.order_history.length - 1 ? 'border-b border-[#eef3f2]' : ''"
                        >
                          <span class="text-[11px] text-[#9bb0af] tabular-nums w-5 shrink-0">{{ idx + 1 }}</span>
                          <span class="text-[12.5px] font-semibold text-[#10393b] font-mono flex-1">{{ h.auftragsnummer }}</span>
                          <span class="text-[12px] text-[#6f8585]">{{ h.leasyback_partner }}</span>
                          <span
                            class="inline-flex items-center gap-1.5 text-[11px] font-bold px-2 py-0.5 rounded-full"
                            :style="`background:${getStatus(h.order_status).bg}; color:${getStatus(h.order_status).fg}`"
                          >
                            <span class="w-[4px] h-[4px] rounded-full bg-current"></span>
                            {{ getStatus(h.order_status).label }}
                          </span>
                          <span class="text-[11.5px] text-[#9bb0af] tabular-nums">
                            {{ formatGermanDate(h.created_at) }}
                          </span>
                        </div>
                      </div>

                      <!-- No history -->
                      <div v-else class="px-5 py-4 text-[12.5px] text-[#9bb0af] text-center">
                        Keine Auftragshistorie vorhanden.
                      </div>
                    </div>
                  </td>
                </tr>
...
<style scoped>
.accordion-enter-active { transition: all 0.22s ease; }
.accordion-leave-active { transition: all 0.18s ease; }
