...
import {
  ...
  orderStatusLabels,
  getVehiclesViewStatus as getStatus,
} from "@/lib/status";
...
async function loadDocuments(vehicleId: string) {
  try {
    ...
    // Extract report_documents from vehicle orders if available
    let reportDocs: any[] = [];

    // First check if vehicle has its own orders with report_documents
    if (vehicle?.orders && vehicle.orders.length > 0) {
      reportDocs = vehicle.orders.flatMap((order: any) =>
        (order.report_documents || []).map((doc: any) => ({
          id: doc.id,
          document_type: doc.document_type,
          file_name: doc.document_title || doc.file_name || "Report",
          created_at: doc.created_at,
          url: doc.signed_url || doc.s3_url,
    ...
    documents.value[vehicleId] = [...vehicleDocs, ...reportDocs];
  } catch (err) {
