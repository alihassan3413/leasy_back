...
<template>
  <div class="flex flex-col gap-5 h-full">
    ...
    <header
      ...
      <div class="flex gap-0.5 bg-[#f4f7f6] p-[3px] rounded-[12px]">
        <button v-for="opt in [{ label: 'Privatkunden', val: 'B2C' }, { label: 'Firmenkunden', val: 'B2B' }]"
          :key="opt.val" @click="viewType = opt.val as 'B2C' | 'B2B'"
          class="text-[12.5px] font-bold px-4 py-1.5 rounded-[9px] transition-all font-[Manrope,sans-serif]" :class="viewType === opt.val
        ...
        <table class="min-w-full border-collapse">
          ...
          <tbody>
            ...
            <tr v-else v-for="u in users" :key="u.user_id"
              ...
              <td class="px-5 py-3.5">
                <span class="inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full" :class="u.is_active
              ...
              <td class="px-3 py-3.5">
                <div class="flex items-center gap-1">
                  <button @click="openAddVehicleModal(u, $event)" class="w-8 h-8 flex items-center justify-center rounded-[9px]
                           text-[#bcccca] hover:bg-[#EF8450] hover:text-white transition-all"
  ...
  <AdminAddVehicleModal :open="showAddVehicleModal" @update:open="showAddVehicleModal = $event"
    :target-user="selectedUser" @vehicle-created="handleVehicleCreated" />
</template>
